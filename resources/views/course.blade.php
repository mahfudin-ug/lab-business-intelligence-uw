    ...
    <div class="col-md-12">
      <div class="card">
        <div class="card-header">
          <div class="card-title">Course's Performance</div>
        </div>
        <div class="card-body">
          <div id="courseChart" style="height: 400px; width: 100%;">
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="card">
        <div class="card-header">
          <div class="card-title">
            <span>Course Clustering based on :</span> 
            ...
            <table id="multi-filter-select" class="display table table-striped table-hover" >
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td>{!! $course->avg_num_grades !!}</td>
                  <td>{!! $course->total_num_grades !!}</td>
                  <td>{!! $course->total_num_courses !!}</td>
...
window.onload = function () {
  ...
  var coursesAvgGpa = {!! $coursesAvgGpa !!}
  var coursesAvgGrade = {!! $coursesAvgGrade !!}

  // Casting the y
  Object.keys(coursesAvgGrade).forEach(k => {
    coursesAvgGpa[k].y = parseFloat(coursesAvgGpa[k].y);
    coursesAvgGrade[k].y = parseFloat(coursesAvgGrade[k].y);
  })

  var courseChart = new CanvasJS.Chart("courseChart", {
    zoomEnabled: true,
    toolTip: {
      shared: true
    },
    ...
    data: [{
        ...
        name: 'GPA',
        showInLegend: true,
        dataPoints: coursesAvgGpa
      },
      {
        type: "stackedColumn",
        name: 'Students/Class',
        showInLegend: true,
        dataPoints: coursesAvgGrade
      }
    ]
  });
  courseChart.render();
